...
function incoming(message, socket) {
  ...
  switch (m.shift()) {
    case "k":
      ...
      if (m.length === 1) {
        ...
        if (socket.permissions) {
          ...
          if (key == process.env.TOKEN_POL32) {
    ...
    case "s":
      ...
      let spawn = true;
      if (receivedBodyID) {
        console.log("Player with an body ID detected! Checking the spawn request body ID.");
        let bodyInfo = sockets.playersReceived.find(i => i.id === receivedBodyID);
        if (bodyInfo) {
            spawn = false;
            if (bodyInfo.checkDednut == true) socket.player = socket.spawnDreadmode(name), socket.player.body.isDreadnought = true; else socket.player = socket.spawn(name);
            ...
            setTimeout(() => {
              for (let def of bodyInfo.definition) {
                if (bodyInfo.checkDednut == true) {
                  ...
                  if (def in Class) socket.player.body.upgrades = [], socket.player.body.define(Class[def]);
                  else if (typeof def === "object") socket.player.body.upgrades = [], socket.player.body.define(def);
                }
                if (
            ...
            for (let score = 0; socket.player.body.skill.score - score >= socket.player.body.skill.levelScore; score += socket.player.body.skill.levelScore, socket.player.body.skill.level += 1) {}
            setTimeout(() => {
...
if (filter.length) {
  ...
  body.invuln = true;
  socket.talk("m", "You will be now invulnerable until you shoot, or wait 10 seconds.");
  body.name = name;
  setTimeout(() => {
