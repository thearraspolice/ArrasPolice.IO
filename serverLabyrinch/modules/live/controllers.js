...
compressMovement = (current, goal) => {
  let offset =
    compressMovementOffsets[
      Math.round(
        (Math.atan2(current.y - goal.y, current.x - goal.x) / (Math.PI * 2)) *
          8 +
          4,
      ) % 8
    ];
...
think(input) {
  if (
    new Vector(
      this.body.x - this.goalDefault.x,
      this.body.y - this.goalDefault.y,
    ).isShorterThan(50)
  ) {
    this.enabled = false;
  }
...
constructor(b, opts = { static: false }) {
  super(b);
  if ("object" != typeof opts.player)
    throw new Error('Required IO Option "player" is not an object');
...
think() {
  ...
  if (this.player.command.autospin) {
    let kk =
      ...
      (this.player.command.rmb ? 0.02 * -1 : 0.02);
    if (this.body.autospinBoost) {
      let thing =
        0.02 * (this.body.autospinBoost * (this.body.skill.spd / 4 + 0.5));
...
think(input) {
  if (input.fire)
    for (let i = 0; i < this.body.guns.length; i++)
      if (!this.onlyIfHasAltFireGun || this.body.guns[i].altFire)
        return { alt: true };
}
}
...
think({ target }) {
  ...
  for (let i = 0; i < this.body.guns.length; i++) {
    ...
    let reloadStat =
        gun.calculator == "necro" || gun.calculator == "fixed reload"
          ? 1
          : (gun.bulletStats === "master" ? this.body.skill : gun.bulletStats)
              .rld,
  ...
  if (
    !readiestGun ||
    (this.timeUntilFire && this.timeUntilFire > lowestReadiness)
  ) {
    return;
  }
...
validate(e, m, mm, sqrRange, sqrRangeMaster) {
  return (
    ...
    (e.type === "miniboss" ||
      e.type === "tank" ||
      e.type === "crasher" ||
      (!this.body.aiSettings.IGNORE_SHAPES && e.type === "food")) &&
    (this.body.aiSettings.BLIND ||
      ((e.x - m.x) * (e.x - m.x) < sqrRange &&
        (e.y - m.y) * (e.y - m.y) < sqrRange)) &&
    (this.body.aiSettings.SKYNET ||
      ((e.x - mm.x) * (e.x - mm.x) < sqrRangeMaster &&
        (e.y - mm.y) * (e.y - mm.y) < sqrRangeMaster))
  );
}
buildList(range) {
...
think(input) {
  ...
  if (!Number.isFinite(tracking)) {
    tracking = this.body.topSpeed + 0.01;
  }
...
think(input) {
  ...
  let offset =
    this.independent && this.body.bond != null ? this.body.bound.angle : 0;
  return {
    target: {
...
think(input) {
  if (
    new Vector(
      this.body.x - this.spot.x,
      this.body.y - this.spot.y,
    ).isShorterThan(50)
  ) {
    this.spot = ran.choose(room.spawnableDefault).loc;
  }
  if (input.goal == null && !this.body.autoOverride) {
    ...
    return {
      target:
        this.lookAtGoal && input.target == null
          ? {
...
think() {
  ...
  let angle =
    this.originAngle +
    this.formula((this.frame += 1 / c.runSpeed), this.body);
  return {
    goal: {
...
constructor(body, opts = {}) {
  ...
  this.body.inverseDist =
    ...
    this.body.dist +
    this.minDistance * this.body.size;
...
think(input) {
  ...
  this.body.dist = Math.min(
    trueMaxDistance,
    Math.max(trueMinDistance, this.body.dist),
  );
  this.body.inverseDist = Math.min(
    trueMaxDistance,
...
think(input) {
  ...
  this.pacify =
    this.body.parent.master.autoOverride ||
    this.body.parent.master.master.autoOverride;
  ...
  if (this.body.alpha != this.targetAlpha) {
    this.body.alpha += util.clamp(
      this.targetAlpha - this.body.alpha,
      -0.05,
      0.05,
    );
    if (this.body.flattenedPhoto)
      this.body.flattenedPhoto.alpha = this.body.alpha;
  }
}
}
