...
global.maxOldDreadFoodSpawn = 0; // look at index.js.
global.maxGreenBasics = 0;
global.maxBlueBasics = 0;
global.fps = "Unknown";
global.minimap = [];
global.entities = [];
global.views = [];
...
global.getSpawnableArea = (team) =>
  ran
    .choose(
      team in room.spawnable && room.spawnable[team].length
        ? room.spawnable[team]
        : room.spawnableDefault,
    )
    .randomInside();
global.getTeamName = (team) =>
  ["BLUE", "GREEN", "RED", "PURPLE", "YELLOW", "ORANGE", "BROWN", "CYAN"][
    -team - 1
  ] || "An unknown team";
global.getTeamColor = (team) =>
  [10, 11, 12, 15, 25, 26, 27, 28][-team - 1] || 3;
...
global.getWeakestTeam = () => {
  ...
  return parseInt(
    !entries.length
      ? -Math.ceil(Math.random() * c.TEAMS)
      : ran.choose(entries)[0],
  );
};
...
global.setSyncedTimeout = (callback, ticks = 0, ...args) =>
  tickEvents.once(tickIndex + Math.round(ticks), () => callback(...args));
...
global.ensureIsClass = (str) => {
  ...
  console.log(Class);
  throw Error(
    `Definition ${str} is attempted to be gotten but does not exist!`,
  );
};
