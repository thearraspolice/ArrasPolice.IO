...
const MC_functions = {
  ...
  gunArray: (callback, count) => {
    if (typeof callback != "function")
      throw new Error(`${callback} isn't type of function`);
    let output = [];
  ...
  enableGuns: (guns, tag = false) =>
    tag
      ? guns
          .filter((gun) => gun._tag == tag)
          .forEach((gun) => (gun._charged = true))
      : guns.forEach((gun) => (gun._charged = true)),
  disableGuns: (guns, tag = false) =>
    tag
      ? guns
          .filter((gun) => gun._tag == tag)
          .forEach((gun) => (gun._charged = false))
      : guns.forEach((gun) => (gun._charged = false)),
  isGunsDisabled: (guns, tag = false) =>
    guns.filter((gun) => gun._charged && (tag ? gun._tag == tag : true))
      .length == 0,
  initGuns: (guns, ...tags) => {
    ...
    for (let i = 0; i < guns.length; i++) {
      const gun = guns[i];
      if (tags.length)
        gun._tag = tags[guns.indexOf(gun)] || tags[tags.length - 1];
      gun._fire = gun.fire;
      gun._charged = true;
  ...
  createDeveloper: (def) => {
    if (typeof def != "object")
      throw new Error(`${def} property suppose to be an object`);
  ...
  createPetal: (petal, def = false) => {
    if (typeof petal != "number")
      throw new Error(`${petal} property suppose to be a number`);
    ...
    let output = def ? MC_functions.deepCopy(def) : [],
      push = {
        ...
        PROPERTIES: {
          SHOOT_SETTINGS: combineStats([MC_stats.statPetal]),
          TYPE: [
            `MC_petal${MC_names.petals[petal]}`,
            {
              ANGLE: petal * 60,
            },
          ],
  ...
  createTail: (tail, def = false) => {
    if (typeof tail != "object")
      throw new Error(`${tail} property suppose to be an object`);
    if (def && typeof def != "object")
      throw new Error(`${def} property suppose to be an object`);
    let output = def ? MC_functions.deepCopy(def) : [],
      push = {
  ...
  createFireRange: (fireRange, def = false) => {
    if (typeof fireRange != "object")
      throw new Error(`${fireRange} property suppose to be an object`);
    if (def && typeof def != "object")
      throw new Error(`${def} property suppose to be an object`);
    let output = def ? MC_functions.deepCopy(def) : [],
      push = {
  ...
  createAnimation: (animation, def) => {
    if (typeof animation != "object")
      throw new Error(`${animation} property suppose to be an object`);
    if (def && typeof def != "object")
      throw new Error(`${def} property suppose to be an object`);
    let output = def ? MC_functions.deepCopy(def) : [],
      pos = undefined;
  ...
  createFirework: (firework) => {
    if (typeof firework != "object")
      throw new Error(`${firework} property suppose to be an object`);
    return {
  ...
  createLaser: (laser) => {
    if (typeof laser != "object")
      throw new Error(`${laser} property suppose to be an object`);
    return {
      PARENT: "bullet",
      COLOR: laser.COLOR ?? MC_names.color,
      SHAPE:
        "M -4.5 1 C -6 1 -6 1 -6 0 C -6 -1 -6 -1 -4.5 -1 H 4 C 4 -1 5 -1 5 0 C 5 1 4 1 4 1 H -4.5",
      BORDERLESS: true,
      ON: [
        {
          event: "tick",
  ...
  create: (poison = false, args, funcs) => {
    ...
    for (let i = 0; i < funcs.length; i++) {
      ...
      if (typeof func != "function")
        throw new Error(`${func} isn't type of function`);
      if (
        event != "death" &&
        event != "tick" &&
        event != "define" &&
    ...
    if (poison) {
      if (Class[name].ON.length && Class[name].ON[0].event == "death")
        throw new Error(`${Class[name]} over-cooked!`);
      Class[name].ON.push({
        event: "death",
        handler: ({ body }) => {
          let instances = [];
          for (let instance of entities) {
            ...
            if (
              ...
              instance.team != body.team &&
              dist2 <= ((body.size / 12) * 100) ** 2 &&
              (instance.type == "tank" || instance.type == "food")
            ) {
  ...
  parse: (def) => {
    ...
    if (!MC_functions.isCompatible(def.BODY))
      throw new Error(`BODY in ${def} class isn't compatible`);
    ...
    for (let i = 0; i < def.ANIMATION.length; i++) {
      ...
      if (item.AUTOFIRE === undefined || !item.TYPE)
    ...
    let output =
      charge > 0
            ...
            tick: (body) => {
              if (MC_functions.isGunsDisabled(body.guns)) {
                if (body._charges < body._maxCharges && !body._tickTime) {
                  ...
                  body._charges++;
                  if (body._charges >= body._maxCharges)
                    MC_functions.enableGuns(body.guns);
                }
        ...
        : {
            fire: (body) => {
              if (
                MC_functions.isGunsDisabled(body.guns, "secondary") &&
                body._tickTime
              ) {
                body._tickTime--;
              }
            ...
            tick: (body) => {
              if (
                MC_functions.isGunsDisabled(body.guns, "main") &&
                body._tickTime
              ) {
                body._tickTime--;
              }
            ...
            define: (body) => {
              body._tickTime = MC_names.ticks;
              MC_functions.initGuns(
                body.guns,
                "main",
                ["secondary", MC_names.gunsCount],
                "main",
              );
              MC_functions.disableGuns(body.guns, "main");
            },
...
const MC_definitions = {
  ...
  Shimu: {
    UPGRADE_TOOLTIP:
      "She isn't as strong as you might think, and no, not outside...",
    GUNS: [
      {
        gun: "Laser",
        event: null,
...
const MC_names = {
  ...
  types: {
    ...
    BigBlast: [MC_stats.statMain, MC_stats.statPounder, MC_stats.statPower],
    Short: [MC_stats.statMain, MC_stats.statPounder, MC_stats.statNoRange],
    Long: [
      MC_stats.statMain,
      MC_stats.statNoRange,
      MC_stats.statNoRecoil,
...
class MC_orbitEntity extends IO {
  constructor(body, opts = {}) {
    ...
    this.body.inverseDist =
      this.norDistance * this.body.size -
      this.body.dist +
      this.norDistance * this.body.size;
    this.body.angle = 0;
  }
  ...
  think(input) {
    this.body.angle +=
      ((this.body.skill.spd * 2 + this.body.aiSettings.SPEED) * Math.PI) / 180;
