  ...
  let cannons = [
    {
      ...
      PROPERTIES: {
        SHOOT_SETTINGS: exports.combineStats([
          g.basic,
          g.pelleter,
          g.power,
          g.twin,
  ...
  for (let gun of type.GUNS) {
    for (let i = 0; i < count; i++) {
      ...
      if (Array.isArray(newgun.POSITION)) {
        ...
        newgun.POSITION.ANGLE =
          (newgun.POSITION.ANGLE ?? 0) + startRotation + fraction * i;
      }
  ...
  for (let gun of output.GUNS) {
    if (gun.PROPERTIES) {
      ...
      if (gun.PROPERTIES.SHOOT_SETTINGS) {
        gun.PROPERTIES.SHOOT_SETTINGS = exports.combineStats([
...
exports.weaponArray = (
  ...
  for (let weapon of weapons) {
    for (let i = 0; i < count; i++) {
      ...
      newWeapon.POSITION[angleKey] =
        (newWeapon.POSITION[angleKey] ?? 0) + angle;
      if (!isTurret) {
        newWeapon.POSITION[delayKey] =
          (newWeapon.POSITION[delayKey] ?? 0) + delay;
        if (!delayOverflow) {
  ...
  addLayer({ gun, turret }, decreaseSides = true, layerScale, MAX_CHILDREN) {
    ...
    let layer = {
      ...
      CONTROLLERS: [
        [
          ...
          {
            independent: true,
            speed: (0.02 / c.runSpeed) * (this.layerID % 2 ? -1 : 1),
          },
        ],
      ],
      MAX_CHILDREN,
    ...
    if (gun) {
      for (let i = 0; i < this.shape; i++) {
        layer.GUNS.push({
          POSITION: gun.POSITION.map(
            (n) => n ?? (360 / this.shape) * (i + 0.5),
          ),
