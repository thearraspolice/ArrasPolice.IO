let fs = require('fs'),
    ...
    other = fs.readdirSync(path.resolve(__dirname, './other')),
    definitionCount = 0,
...
console.log(`Loading ${other.length} others...`);
for (let filename of other) {
    if (!filename.endsWith('.js')) continue;
    
    console.log(`Loading other: ${filename}`);
    let result = require('./other/' + filename);
    if ('function' === typeof result) {
        result({ Config: c, Events: events });
    }
}

let otherLoadEnd = Date.now();
console.log("Loaded others in " + (otherLoadEnd - addonsLoadEnd - definitionGroupsLoadEnd) + " milliseconds. \n");
...
console.log(`Added ${other.length} others to the definitions.`);
console.log(`Successfully generated 'definitions.js'.`)
