...
class Gun {
  constructor(body, info) {
    ...
    if (info.PROPERTIES != null) {
      ...
      if (info.PROPERTIES.COLOR != null) {
        if (
          typeof info.PROPERTIES.COLOR === "number" ||
          typeof info.PROPERTIES.COLOR === "string"
        ) {
          ...
          this.colorUnboxed = {
            ...
            allowBrightnessInvert:
              info.PROPERTIES.COLOR.ALLOW_BRIGHTNESS_INVERT ?? false,
          };
        this.color =
          this.colorUnboxed.base +
          " " +
  ...
  fire(gx, gy, sk) {
    ...
    if (this.independentChildren) {
      ...
      o.life();
      this.master.ON(undefined, this.altFire ? "altFire" : "fire", {
        gun: this,
        store: this.store,
  ...
  bulletInit(o) {
    ...
    if (
      !o.color ||
      o.colorUnboxed.base == "-1" ||
      o.colorUnboxed.base == "mirror"
    ) {
...
class antiNaN {
  constructor(me) {
    ...
    this.amNaN = (me) =>
      [me.x, me.y, me.velocity.x, me.velocity.y, me.accel.x, me.accel.y].some(
        isNaN,
      );
  }
  update() {
...
function getValidated(obj, prop, allowedType, from, optional = true) {
  ...
  throw new TypeError(
    `${from} property ${prop} is of type ${type} instead of type ${allowedType}`,
  );
}
...
class StatusEffect extends EventEmitter {
  constructor(duration = 0, multipliers = {}, tick = (a) => a) {
    ...
    this.acceleration = getValidated(
      multipliers,
      "acceleration",
      "number",
      labelThing,
    ...
    this.pushability = getValidated(
      multipliers,
      "pushability",
      "number",
      labelThing,
    );
    this.recoilReceived = getValidated(
      multipliers,
      "recoilReceived",
      "number",
      labelThing,
    ...
    this.tick = getValidated(
      { tick },
      "tick",
      "function",
      "StatusEffect's argument",
    );
  }
}
...
class Prop {
  ...
  define(def) {
    ...
    this.imageInterpolation =
      set.IMAGE_INTERPOLATION != null ? set.IMAGE_INTERPOLATION : "bilinear";
    if (set.COLOR != null) {
      if (typeof set.COLOR === "number" || typeof set.COLOR === "string")
        ...
        if (set.COLOR.HUE_SHIFT != null)
          this.colorUnboxed.hueShift = set.COLOR.HUE_SHIFT;
        if (set.COLOR.SATURATION_SHIFT != null)
          this.colorUnboxed.saturationShift = set.COLOR.SATURATION_SHIFT;
        if (set.COLOR.BRIGHTNESS_SHIFT != null)
          this.colorUnboxed.brightnessShift = set.COLOR.BRIGHTNESS_SHIFT;
        if (set.COLOR.ALLOW_BRIGHTNESS_INVERT != null)
          this.colorUnboxed.allowBrightnessInvert =
            set.COLOR.ALLOW_BRIGHTNESS_INVERT;
      }
      this.color =
        this.colorUnboxed.base +
        " " +
...
class Entity extends EventEmitter {
  constructor(position, master) {
    ...
    this.invuln = false;
    this.hasAcended = false;
    this.alwaysHasInvuln = false;
    this.alpha = 1;
    this.strokeWidth = 1;
  ...
  life() {
    ...
    if (
      !this.damageReceived &&
      this.velocity.x ** 2 + this.velocity.y ** 2 <= 0.1
    ) {
    ...
    for (let i = 0; i < this.statusEffects.length; i++) {
      ...
      if (
        entry.effect.tick &&
        entry.effect.tick(this, entry.effect, entry.durationLeftover)
      ) {
        needsBodyAttribRefresh = true;
      }
    }
    ...
    // Think
    let faucet =
        this.settings.independent || this.source == null || this.source === this
          ? {}
          : this.source.control,
      b = {
    ...
    for (let i = 0; i < this.controllers.length; i++) {
      let AI = this.controllers[i],
        a = AI.think(b);
      if (a != null) {
        if (a.target != null && (b.target == null || AI.acceptsFromTop))
          b.target = a.target;
        if (a.goal != null && (b.goal == null || AI.acceptsFromTop))
          b.goal = a.goal;
        if (a.fire != null && (b.fire == null || AI.acceptsFromTop))
          b.fire = a.fire;
        if (a.main != null && (b.main == null || AI.acceptsFromTop))
          b.main = a.main;
        if (a.alt != null && (b.alt == null || AI.acceptsFromTop))
          b.alt = a.alt;
        if (a.power != null && (b.power == null || AI.acceptsFromTop))
          b.power = a.power;
      }
    }
    ...
    if (
      !this.alwaysHasInvuln &&
      this.invuln &&
      (this.control.goal.x !== this.x || this.control.goal.y !== this.y)
    ) {
      this.invuln = false;
    }
  ...
  become(player, dom = false) {
    this.addController(
      new ioTypes.listenToPlayer(this, { player, static: dom }),
    );
  ...
  define(defs, emitEvent = true) {
    ...
    if (set.MIRROR_MASTER_ANGLE != null)
      this.settings.mirrorMasterAngle = set.MIRROR_MASTER_ANGLE;
    ...
    if (set.NECRO != null)
      this.settings.necroTypes = Array.isArray(set.NECRO)
        ? set.NECRO
        : set.NECRO
          ? [this.shape]
          : [];
    ...
    if (set.BODY != null) {
      ...
      if (set.BODY.RECOIL_MULTIPLIER != null)
        this.RECOIL_MULTIPLIER = set.BODY.RECOIL_MULTIPLIER;
    ...
    if (set.REROOT_UPGRADE_TREE)
      this.rerootUpgradeTree = set.REROOT_UPGRADE_TREE;
    ...
    if (!this.allDefs) this.allDefs = [];
    this.defs = [];
    for (let def of defs) {
      this.defs.push(def);
    }
    this.allDefs.push(defs);
    ...
    for (let branch = 1; branch < defs.length; branch++) {
      ...
      if (set.LABEL != null && set.LABEL.length > 0)
        this.label = this.label + "-" + set.LABEL;
      if (set.MAX_CHILDREN != null) this.maxChildren += set.MAX_CHILDREN;
  ...
  chooseUpgradeFromBranch(remaining) {
    if (remaining > 0) {
      ...
      if (branchUgrades.length == 0)
        // For when the branch has no upgrades
        this.chooseUpgradeFromBranch(remaining - 1);
    } else {
      ...
      switch (onPairs.event) {
        ...
        case "collide":
          if (eventName == "collide")
            onPairs.handler({ instance: value.instance, other: value.other });
          break;
        case "damage":
          if (eventName == "damage")
            onPairs.handler({
        ...
        case "upgrade":
          if (eventName == "upgrade")
            onPairs.handler({ body: this, oldEntity: value.oldEntity });
          break;
        case "tick":
  ...
  refreshBodyAttributes() {
    ...
    for (let i = 0; i < this.statusEffects.length; i++) {
      ...
      if (effect.acceleration != null)
        accelerationMultiplier *= effect.acceleration;
      ...
      if (effect.penetration != null)
        penetrationMultiplier *= effect.penetration;
      ...
      if (effect.pushability != null)
        pushabilityMultiplier *= effect.pushability;
      if (effect.recoilReceived != null)
        recoilReceivedMultiplier *= effect.recoilReceived;
      if (effect.size != null) sizeMultiplier *= effect.size;
    }

    let speedReduce = Math.pow(this.size / (this.coreSize || this.SIZE), 1);
    this.acceleration =
      (accelerationMultiplier * c.runSpeed * this.ACCELERATION) / speedReduce;
    if (this.settings.reloadToAcceleration) this.acceleration *= this.skill.acl;
    this.topSpeed =
      (topSpeedMultiplier * c.runSpeed * this.SPEED * this.skill.mob) /
      speedReduce;
    if (this.settings.reloadToAcceleration)
      this.topSpeed /= Math.sqrt(this.skill.acl);
    this.health.set(
      ((this.settings.healthWithLevel ? 2 * this.level : 0) + this.HEALTH) *
        this.skill.hlt *
        healthMultiplier,
    );
    this.health.resist = 1 - 1 / Math.max(1, this.RESIST + this.skill.brst);
    this.shield.set(
      ((this.settings.healthWithLevel ? 0.6 * this.level : 0) + this.SHIELD) *
        this.skill.shi,
      Math.max(
        0,
        ((this.settings.healthWithLevel ? 0.006 * this.level : 0) + 1) *
          this.REGEN *
          this.skill.rgn *
          regenMultiplier,
      ),
  ...
  bindToMaster(position, bond, isInvulnerable) {
    ...
    if (isInvulnerable)
      this.on("dead", () => {
        util.remove(this.master.turrets, this.master.turrets.indexOf(this));
      });
  ...
  camera(tur = false) {
    ...
    return {
      ...
      twiggle:
        forceTwiggle.includes(this.facingType[0]) ||
        (this.facingType[0] === "locksFacing" && this.control.alt),
      layer: this.layerID
        ? this.layerID
        : this.bond != null
          ? this.bound.layer
  ...
  upgrade(number) {
    ...
    if (
      ...
      if (
        this.colorUnboxed.base == "-1" ||
        this.colorUnboxed.base == "mirror"
      ) {
        this.colorUnboxed.base = getTeamColor(
          c.MODE == "ffa" || c.GROUPS ? TEAM_RED : this.team,
        );
        this.compressColor();
      }
      ...
      for (let def of this.defs) {
        ...
        if (typeof def.TOOLTIP == "string" && def.TOOLTIP.length > 0) {
          let tooltips = Array.isArray(def.TOOLTIP)
            ? def.TOOLTIP
            : [def.TOOLTIP];
  ...
  move() {
    ...
    switch (type) {
      ...
      case "desmos":
        this.damp = 0;
        if (this.waveReversed == null)
          this.waveReversed = this.master.control.alt ? -1 : 1;
        ...
        let waveX =
          this.maxSpeed *
          5 *
          Math.cos(((this.RANGE - this.range) / (args.period ?? 4)) * 2);
        let waveY =
          (args.amplitude ?? 15) *
          Math.cos((this.RANGE - this.range) / (args.period ?? 4)) *
          this.waveReversed *
          (args.invert ? -1 : 1);
        this.x +=
          Math.cos(this.waveAngle) * waveX - Math.sin(this.waveAngle) * waveY;
        this.y +=
          Math.sin(this.waveAngle) * waveX + Math.cos(this.waveAngle) * waveY;
        break;
    }
    ...
    switch (type) {
      ...
      case "bound":
        let givenangle,
          ...
          slowness = this.settings.mirrorMasterAngle
            ? 1
            : (args.slowness ?? 4) / c.runSpeed;
    ...
    if (!this.settings.canGoOutsideRoom) {
      if (c.ARENA_TYPE === "circle") {
        ...
        if (dist > room.width / 2) {
          let strength =
            ((dist - room.width / 2) * c.ROOM_BOUND_FORCE) / (c.runSpeed * 750);
  ...
  contemplationOfMortality() {
    if (this.invuln || this.alwaysHasInvuln || this.godmode) {
      this.damageRecieved = 0;
      return 0;
    }
    if (this.damageRecieved > 0) {
      ...
      this.onDef != null
        ? this.ON(undefined, "damage", { damageInflictor, damageTool })
        : null;
    ...
    if (this.isDead()) {
      ...
      this.onDef != null
        ? this.ON(this.onDef, "death", { killers, killTools })
        : null;
      ...
      if (this.id === room.topPlayerID) {
        ...
        if (notJustFood) {
          ...
          for (let i = 0; i < killers.length; i++) {
            usurptText += " ";
            usurptText +=
              killers[i].name === "" ? "an unnamed player" : killers[i].name;
            usurptText += " and";
          }
