...
function AddMoreCounts() {
  visitCount += 1;
  setTimeout(() => {
    util.log("Players have visted: " + visitCount);
  }, 10000);
}
// Closing the socket
function close(socket) {
  ...
  if (index != -1) {
    ...
    util.log(
      "[INFO] " +
        (player.body
          ? "User " + player.body.name
          : "A user without an entity") +
        " disconnected!",
    );
...
// Being banned
function ban(socket, reason) {
  let time = Date.now();
  util.warn((reason || "No reason given.") + " Banning.");
  let s = sockets.clients.filter((c) => c.ip === socket.ip);
  ...
  bans.push({
    ...
    time: time,
    name:
      (socket.player && socket.player.body && socket.player.body.name) ||
      "Unnamed",
    reason: reason,
  });
}
...
function incoming(message, socket) {
  ...
  switch (m.shift()) {
    case "k":
      if (!global.doNotResetBan) {
        socket.talk("banreset");
      } else {
        socket.talk("checkban");
      }
      socket.talk("banmsgperm"); // check it if they are permant banned.
      // key verification
      if (m.length > 1) {
      ...
      if (m.length === 1) {
        ...
        if (socket.permissions) {
          if (key == process.env.TOKEN_DEVELOPER) {
            util.log("An developer has joined.");
          } else if (key == process.env.TOKEN_POL32) {
            util.log("An beta tester has joined.");
          }
        }
        socket.key = key;
      }
    ...
    case "s":
      ...
      let permB = permBans.find((bannedIP) => bannedIP === socket.ip);
      if (permB) {
        socket.talk("banmsg2");
        socket.kick("Permanent Banned player found!");
        socket.talk("banmsgperm");
      }
      ...
      let needsRoom = m[1];
      let autoLVLup = m[2];
      let receivedBodyID = m[3];
      if (receivedBodyID) receivedBodyID = receivedBodyID.replace(name, "");
      // Verify it
      ...
      if (receivedBodyID) {
        console.log(
          "Player with an body ID detected! Checking the spawn request body ID.",
        );
        let bodyInfo = sockets.playersReceived.find(
          (i) => i.id === receivedBodyID,
        );
        if (bodyInfo) {
          spawn = false;
          if (bodyInfo.checkDednut == true)
            (socket.player = socket.spawnDreadmode(name)),
              (socket.player.body.isDreadnought = true);
          else socket.player = socket.spawn(name);
          socket.player.body.upgrades = [];
          setTimeout(() => {
            socket.player.body.allDefs = [];
          }, 500);
          setTimeout(() => {
            for (let def of bodyInfo.definition) {
              if (bodyInfo.checkDednut == true) {
                if (def in Class) socket.player.body.define(Class[def]);
                  ...
                  socket.player.body.define(def);
              } else {
                if (def in Class)
                  (socket.player.body.upgrades = []),
                    socket.player.body.define(Class[def]);
                else if (typeof def === "object")
                  (socket.player.body.upgrades = []),
                    socket.player.body.define(def);
              }
              if (
                socket.player.body.colorUnboxed.base == "-1" ||
                socket.player.body.colorUnboxed.base == "mirror"
              ) {
                socket.player.body.colorUnboxed.base = getTeamColor(
                  c.MODE == "ffa" || c.GROUPS
                    ? TEAM_RED
                    : socket.player.body.team,
                );
                socket.player.body.compressColor();
              }
            }
          }, 1000);
          ...
          for (
            ...
            socket.player.body.skill.levelScore;
            score += socket.player.body.skill.levelScore,
              socket.player.body.skill.level += 1
          ) {}
          setTimeout(() => {
          ...
          if (
            ...
            socket.player.body.colorUnboxed.base = getTeamColor(
              c.MODE == "ffa" || c.GROUPS ? TEAM_RED : socket.player.body.team,
            );
            socket.player.body.compressColor();
          }
          socket.player.body.name = bodyInfo.name;
          console.log("ID is valid and the body has been successfully saved.");
          util.remove(
            sockets.playersReceived,
            sockets.playersReceived.indexOf(bodyInfo),
          );
        } else {
          console.log("Invalid ID! body transfer aborded.");
        }
      }
      if (spawn) {
        ...
        if (autoLVLup) {
          while (socket.player.body.skill.level < c.LEVEL_CHEAT_CAP) {
            socket.player.body.skill.score +=
              socket.player.body.skill.levelScore;
      ...
      socket.talk(
        ...
        room.width,
        room.height,
        JSON.stringify(room.setup.map((x) => x.map((t) => t.color))),
      ...
      // Log it
      util.log(
        `[INFO] ${m[0]} ${needsRoom ? "joined" : "rejoined"} the game on team ${socket.player.body.team}! Players: ${players.length}`,
      );
      if (needsRoom) {
        AddMoreCounts();
      }
      break;
    case "resetEntities":
      sockets.resetEntities();
      break;
    case "#": {
      if (!player.body) return 1;
      runKeyCommand(socket, m[0]);
      break;
    }
    case "S":
      // clock syncing
    ...
    case "U":
      ...
      break;
    case "x":
      // console.log(socket.player.body.defs);
      // console.log("all: " + socket.player.body.allDefs);
...
*/
      ...
      if (player.body != null) {
        ...
        do {
          ...
          limit-- &&
          max &&
          player.body.skill.points &&
          player.body.skill.amount(stat) < player.body.skill.cap(stat)
        );
      }
      break;
    case "L":
      ...
      if (
        ...
        (socket.permissions && socket.permissions.infiniteLevelUp)
      ) {
    ...
    case "0":
      ...
      if (
        ...
        if (
          player.body.colorUnboxed.base == "-1" ||
          player.body.colorUnboxed.base == "mirror"
        ) {
          player.body.colorUnboxed.base = getTeamColor(
            c.MODE == "ffa" || c.GROUPS ? TEAM_RED : player.body.team,
          );
    ...
    case "A":
      ...
      for (let i = 0; i < entities.length; i++) {
        ...
        if (entry.isDominator || entry.isMothership || entry.isArenaCloser)
          possible.push(entry);
        if (
          c.MODE === "tdm" &&
      ...
      socket.talk(
        "m",
        `You are now spectating ${entity.name.length ? entity.name : "An unnamed player"}! (${entity.label})`,
      );
      break;
    /*case "ban": {
      if (
        player.body != null &&
        socket.permissions &&
        socket.permissions.infiniteLevelUp
      ) {
        entities.forEach((o) => {
          if (
            (o !== player.body) != null &&
            ...
            if (o.isPlayer) {
              ...
              socket.talk("m", "Player banned.");
              global.doNotResetBan = true;
              o.socket.talk("banmode");
            }
          }
        });
      }
    ...
    */
    case "H":
      ...
      if (body.underControl) {
        if (c.DOMINATOR_LOOP) {
          ...
          return 1;
        } else if (c.BASIC_ATTACK) {
          body.giveUp(player);
          socket.talk("m", "You have relinquished control of the space station.");
          return 1;
        } else {
          body.giveUp(player);
          socket.talk(
            "m",
            "You have relinquished control of the special tank.",
          );
          return 1;
        }
      }
      if (c.MOTHERSHIP_LOOP) {
        ...
        player.body.sendMessage(
          "Press F to relinquish control of the mothership.",
        );
        ...
        player.body.sendMessage(
          "Press F to relinquish control of the dominator.",
        );
      } else if (c.BASIC_ATTACK) {
        let basics = entities
          .map((entry) => {
        ...
        if (!basics.length) {
          socket.talk(
            "m",
            "There are no space stations available that are on your team!",
          );
          return 1;
        }
        let basic = basics.shift();
        basic.controllers = [];
        basic.underControl = true;
        player.body = basic;
        body.kill();
        player.body.become(player);
        player.body.FOV += 0.5;
        player.body.refreshBodyAttributes();
        player.body.name = body.name;
        player.body.sendMessage("You are now controlling the space station.");
        player.body.sendMessage("Press F to relinquish control of the space station.");
      } else {
        socket.talk(
          "m",
          "There are no special tanks in this mode that you can control.",
        );
      }
      break;
    ...
    case "M":
      ...
      chats[id].unshift({
        message,
        expires: Date.now() + c.CHAT_MESSAGE_DURATION,
      });
...
const spawn = (socket, name) => {
  ...
  if (c.MODE == "tdm") {
    ...
    if (
      player.team == null ||
      (player.team !== team && global.defeatedTeams.includes(player.team))
    ) {
      player.team = team;
    }
    if (socket.party) {
  ...
  if (filter.length) {
    ...
    body = recover.body;
    body.controllers = body.controllers.filter(
      (con) => !(con instanceof ioTypes.listenToPlayer),
    );
    body.become(player);
  ...
  switch (c.MODE) {
    case "tdm":
      if (
        body.colorUnboxed.base == "-1" ||
        body.colorUnboxed.base == "mirror"
      ) {
  ...
  player.teamColor =
    (!c.RANDOM_COLORS && (c.MODE === "ffa" || c.GROUPS)
      ? 10
      : getTeamColor(body.team)) + " 0 1 0 false"; // blue
...
const spawnDreadmode = (socket, name) => {
  let player = {},
    loc = {};
  loc = getSpawnableArea(player.team);
  ...
  if (filter.length) {
    ...
    body.protect();
    body.isPlayer = true;
    body.isDreadnought = true;
    body.team = -9;
    body.x = room.width / 2;
    body.y = room.height / 2;
    body.define(c.SPAWN_CLASS);
    if (socket.permissions && socket.permissions.nameColor) {
      body.nameColor = socket.permissions.nameColor;
      socket.talk("z", body.nameColor);
...
function perspective(e, player, data) {
  if (player.body != null) {
    ...
    if (player.body.alwaysHasInvuln) {
      data[16];
    }
      ...
      all.push({
        ...
        data: [
          my.type === "wall" || my.isMothership
            ? my.shape === 4 || my.shapeData == "M 1 1 L -1 1 L -1 -1 L 1 -1 Z"
              ? 2
              : 1
            : 0,
...
const sockets = {
  ...
  sendToServer: (socket, server) => {
    ...
    if (socket.player.body.isTravelling) return;
    socket.player.body.isTravelling = true;
    console.log("A player is trying to travel...");
    let id = Math.floor(Math.random() * 0xffffffff)
      .toString(16)
      .padStart(8, "0");
    console.log("Sending player to server", server);
    console.log("ID:", id);
    fetch(`${server}/api/sendPlayer`, {
      ...
      body: JSON.stringify({
        ...
        definition: socket.player.body.allDefs.map((def) =>
          def.map((d) => Object.keys(Class).find((k) => Class[k] === d) || d),
        ),
      ...
      .then(async (r) => {
        if (r.status === 200) {
          ...
          let transform = setInterval(() => {
            if (socket.player.body == null) return clearInterval(transform);
            if (socket.player.body.SIZE > 2) {
              socket.player.body.SPEED = 1;
              socket.player.body.FOV *= 4 / 5;
              socket.player.body.SIZE *= 4 / 5;
            } else {
              socket.talk(
                "t",
          ...
          console.log(
            "Failed to contact the other server.",
            r.statusText,
            await r.text(),
          );
        }
  ...
  connect: (socket, req) => {
    ...
    socket.spawnDreadmode = (name) => spawnDreadmode(socket, name);
    ...
    //account for proxies (existing code)
    ...
    if (same >= 2 && ips[0]) {
      if (connectedIPs.has(ips[0])) {
        connectedIPs.delete(ips[0]); // Remove existing entry
      }
      socket.talk("toomanytabswtf");
      socket.kick("Second Alt detected!");
    }

    connectedIPs.add(ips[0]); // Add new entry (optional, for tracking)

    // Rest of the code (assign IP, log, etc.)
    socket.ip = ips[0];
