let fs = require('fs'),
    path = require('path'),
    groups = fs.readdirSync(path.resolve(__dirname, '../../../server/modules/definitions/groups')),
    addons = fs.readdirSync(path.resolve(__dirname, '../../../server/modules/definitions/addons')),
    definitionCount = 0,
...
for (let filename of groups) {
    ...
    let group = require('../../../server/modules/definitions/groups/' + filename);
...
for (let filename of addons) {
    ...
    let result = require('../../../server/modules/definitions/addons/' + filename);
