...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
  ...
  function startGame() {
    ...
    settings.graphical.fancyAnimations =
      !document.getElementById("optFancy").checked;
    settings.graphical.centerTank =
      document.getElementById("centerTank").checked;
    ...
    settings.lag.unresponsive =
      document.getElementById("optPredictive").checked;
    settings.graphical.screenshotMode =
      document.getElementById("optScreenshotMode").checked;
    settings.graphical.coloredHealthbars =
      document.getElementById("coloredHealthbars").checked;
    settings.graphical.seperatedHealthbars = document.getElementById(
      "seperatedHealthbars",
    ).checked;
...
context.lineJoin = "round";
  ...
  const drawEntity = (
    ...
    let fade = turretInfo ? 1 : render.status.getFade(),
      ...
      initStrokeWidth =
        lineWidthMult *
        Math.max(
          settings.graphical.mininumBorderChunk,
          ratio * settings.graphical.borderChunk,
        );
    source.guns.update();
    ...
    if (
      ...
      context.canvas.width = context.canvas.height =
        ((drawSize * m.position.axis) / ratio) * 2;
      xx =
        context.canvas.width / 2 -
        (drawSize * m.position.axis * m.position.middle.x * Math.cos(rot)) / 4;
      yy =
        context.canvas.height / 2 -
        (drawSize * m.position.axis * m.position.middle.y * Math.sin(rot)) / 4;
    } else {
    ...
    for (let i = 0; i < source.guns.length; i++) {
      ...
      if (!g.drawAbove) {
        ...
        gameDraw.setColor(
          context,
          gameDraw.mixColors(gunColor, render.status.getColor(), blend),
        );
        drawTrapezoid(
          context,
          xx + drawSize * gx,
    ...
    if (instance.drawsHealth) {
      ...
      if (health < 0.99 || shield < 0.99) {
        ...
        drawBar(
          ...
          yy + barWidth * settings.graphical.seperatedHealthbars,
          barWidth,
          col,
        );
        ...
        if (shield || settings.graphical.seperatedHealthbars) {
          if (!settings.graphical.seperatedHealthbars)
  ...
  function drawEntityIcon(
    ...
    ctx.fillStyle =
      picture.upgradeColor != null
        ? gameDraw.getColor(picture.upgradeColor)
        : gameDraw.getColor(
            (colorIndex > 18 ? colorIndex - 19 : colorIndex).toString(),
          );
    ...
    ctx.fillStyle =
      picture.upgradeColor != null
        ? gameDraw.getColor(picture.upgradeColor)
        : gameDraw.getColor((colorIndex - 9).toString());
  ...
  function drawFloor(px, py, ratio) {
    ...
    for (let i = 0; i < H; i++) {
      ...
      for (let j = 0; j < W; j++) {
        ...
        let tile = row[j];
        ctx.globalAlpha = 1;
        ctx.fillStyle = settings.graphical.screenshotMode
          ? color.guiwhite
          : color.white;
        ...
        ctx.fillStyle = settings.graphical.screenshotMode
          ? color.guiwhite
          : gameDraw.modifyColor(tile);
        ctx.fillRect(left, top, right - left, bottom - top);
      }
    }
    ctx.lineWidth = 1.5;
    ctx.strokeStyle = settings.graphical.screenshotMode
      ? color.guiwhite
      : color.guiblack;
  ...
  function drawUpgradeTree(spacing, alcoveSize) {
    if (global.died) {
      ...
      global.scrollX =
        ...
        global.fixedScrollX =
        global.fixedScrollY =
        global.scrollVelocityY =
        global.scrollVelocityX =
          0;
      global.treeScale = 1;
      return;
    }
    ...
    for (let { x, y, colorIndex, index } of tiles) {
      ...
      if (
        ax < -tileSize ||
        ax > global.screenWidth + tileSize ||
        ay < -tileSize ||
        ay > global.screenHeight + tileSize
      )
        continue;
      drawEntityIcon(
        index.toString(),
    ...
    let text =
      "Arrow keys to navigate the class tree. Shift to navigate faster. Scroll wheel (or +/- keys) to zoom in/out.";
  ...
  function drawSelfInfo(spacing, alcoveSize, max) {
    ...
    drawBar(
      ...
      x + len,
      y + height / 2,
      height + settings.graphical.barChunk,
      color.black,
    );
    drawBar(
      x,
      x + len,
      y + height / 2,
      height - settings.graphical.barChunk / 4,
      color.grey,
    );
    drawBar(
      x,
    ...
    drawBar(
      ...
      x + len * 0.9,
      y + height / 2,
      height - 3 - settings.graphical.barChunk / 4,
      color.grey,
    );
    drawBar(
      x + len * 0.1,
    ...
    drawText(
      "Score: " + util.formatLargeNumber(Math.floor(gui.__s.getScore())),
      x + len / 2,
      y + height / 2 + 1,
      height - 3.5,
      color.guiwhite,
  ...
  function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    // Text
    if (global.showDebug) {
      drawText("Test", x + len, y - 50 - 5 * 14 - 2, 15, "#B6E57C", "right");
      drawText(
      ...
      if (upgradeHoverIndex > -1 && upgradeHoverIndex < gui.upgrades.length) {
        ...
        if (picture.upgradeTooltip.length > 0) {
          let boxWidth = measureText(picture.name, alcoveSize / 10),
            boxX =
              (global.mouse.x * global.screenWidth) / window.canvas.width + 2,
            boxY =
              (global.mouse.y * global.screenHeight) / window.canvas.height + 2,
            boxPadding = 6,
  ...
  const gameDrawDead = () => {
    ...
    drawText(
      "Level " + gui.__s.getLevel() + " " + picture.name,
      x - 170,
      y - 30,
      24,
      color.guiwhite,
